...
export class MasterElector {
  ...
  tryBecomeMaster(): Promise<void> {
    return this.persistence
      .runTransaction('become master', txn => {
  ...
  private becomeMaster(txn: PersistenceTransaction): PersistencePromise<void> {
    ...
    return ownersStore(txn).put(
      'owner',
      new DbOwner(this.instanceId, Date.now() + 5000)
    );
  }

  private getCurrentMaster(
    txn: PersistenceTransaction
  ): PersistencePromise<DbOwner | null> {
    return ownersStore(txn)
      .get('owner')
      .next(dbOwner => {
...
function ownersStore(
  ...
  return getStore<DbOwnerKey, DbOwner>(txn, DbOwner.store);
}
